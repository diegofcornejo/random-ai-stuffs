...
Group all current changes into meaningful semantic commits, squash them into a clean history in the current branch.
...
- Use `-c commit.gpgsign=false` with `git commit` to avoid signing commits.
